...
const CheckOutComp = () => {
  ...
  // handle payment
  const handlePayment = () => {
    dispatch({type:"Cart_Data" , data : {side_cart : false}})
    navigate('/cart')
  } 
  ...
  return state.items.length ? (
    <div style={{boxShadow: '5px 5px 15px rgba(0, 0, 0, 0.3)'}} className=' px-4 py-6 flex flex-col gap-y-4'>
        ...
        <button onClick={handlePayment} className='bg-secondarycolor text-white rounded-full text-sm py-2 hover:bg-maincolor'>إتمام الشراء</button>
