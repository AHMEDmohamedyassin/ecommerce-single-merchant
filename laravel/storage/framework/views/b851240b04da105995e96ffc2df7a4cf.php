...
<html lang="ar" dir="rtl">
  <head>
    ...
    <?php 
      $js_dir = public_path('/front/static/js');
      $css_dir = public_path('/front/static/css');
      $tags = '';
      ...
      if(is_dir($js_dir)){
        ...
        foreach ($files as $key => $value) {
          if(preg_match("/^main.*\.js$/", $value))
            $tags = "<script defer src=". asset("/front/static/js/{$value}") . "></script>";
        }
      }
      ...
      if(is_dir($css_dir)){
        ...
        foreach ($files as $key => $value) {
          if(preg_match("/^main.*\.css$/", $value))
            $tags .= "<link href=". asset('/front/static/css/'. $value) ." rel='stylesheet'>";
        }
      }
